...
const part1Schema = new mongoose.Schema({
  ...
  audioUrl: {
    ...
    match: [/^\/audio\/listening\/.+$/, 'Đường dẫn audio không hợp lệ']
  },
  imageUrl: {
    type: String,
    required: [true, 'Đường dẫn hình ảnh là bắt buộc'],
    match: [/^\/images\/listening\/.+$/, 'Đường dẫn hình ảnh không hợp lệ']
  },
  paragraph: {
    type: String,
...
const part3Schema = new mongoose.Schema({
  ...
  diagramUrl: {
    type: String,
    match: [/^\/images\/listening\/.+$/, 'Đường dẫn biểu đồ không hợp lệ']
  },
  questions: {
    type: [questionSchema],
